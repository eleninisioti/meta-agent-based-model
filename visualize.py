...
def plot_meta_model(log, project_dir):
  plt.plot(range(len(log["fitness"])), log["fitness"])
  plt.xlabel("Time step, $t$")
  plt.ylabel("Average fitness, $\\bar{F}$")
  if not os.path.exists(project_dir + "/plots"):
    os.makedirs(project_dir + "/plots")
  plt.savefig(project_dir + "/plots/avg_fitness.png")
  plt.savefig(project_dir + "/plots/avg_fitness.pdf")
  plt.clf()

  for timestep, strategies in enumerate(log["strategies"]):
    plt.hist(strategies, density=True)
    plt.savefig(project_dir + "/plots/strategies_" + str(timestep) + ".png")
    plt.savefig(project_dir + "/plots/strategies_" + str(timestep) + ".pdf")
    plt.clf()
